...
void CFPSPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	int nShowFPSMode = cl_showfps.GetInt();

	if (nShowFPSMode == 1)
	{
		m_hFont = pScheme->GetFont("DefaultFixedOutlineFPS");
	}
	else
	{
	...
	else if ( nFPSMode && realFrameTime > 0.0 )
	{
		if ( m_lastRealTime != -1.0f )
		{
			...
			else
			{
				...
				float frameMS = realFrameTime * 1000.0f;
				GetFPSColor(nFps, ucColor);
				g_pMatSystemSurface->DrawColoredText(m_hFont, x, 2, ucColor[0], ucColor[1], ucColor[2], 255, "%3i fps", nFps);
