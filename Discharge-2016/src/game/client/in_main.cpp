...
void CInput::CalcPlayerAngle(CUserCmd *cmd)
{
	...
	UTIL_TraceLine(MainViewOrigin(0), MainViewOrigin(0) + MainViewForward(0) * MAX_TRACE_LENGTH, MASK_SHOT, pl, COLLISION_GROUP_NONE, &tr);
	...
	float flMinForward = abs(DotProduct(MainViewForward(0), eyePos - MainViewOrigin(0))) + 32.0f;
	Vector vecTrace = tr.endpos - tr.startpos;
	float flLenOld = vecTrace.NormalizeInPlace();
	float flLen = max(flMinForward, flLenOld);
	vecTrace *= flLen;

	Vector vecFinalDir = MainViewOrigin(0) + vecTrace - eyePos; //eyePos;
