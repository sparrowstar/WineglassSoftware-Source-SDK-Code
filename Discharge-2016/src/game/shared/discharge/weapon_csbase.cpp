...
CDC_Player* CWeaponCSBase::GetPlayerOwner() const
{
	return dynamic_cast< CDC_Player* >( GetOwner() );
}

void CWeaponCSBase::SecondaryAttack( void )
{
#ifndef CLIENT_DLL
	CDC_Player *pPlayer = GetPlayerOwner();
...
{
	CDC_Player *pOwner = GetPlayerOwner();
...
void CWeaponCSBase::DefaultTouch(CBaseEntity *pOther)
{
	...
	void CWeaponCSBase::DrawCrosshair()
	{
		...
		CDC_Player* pPlayer = (CDC_Player*)C_BasePlayer::GetLocalPlayer();
	...
	bool CWeaponCSBase::OnFireEvent( C_BaseViewModel *pViewModel, const Vector& origin, const QAngle& angles, int event, const char *options )
	{
		if( event == 5001 )
		{
			C_DC_Player *pPlayer = ToDCPlayer( GetOwner() );
	...
	{
		CDC_Player *pPlayer = dynamic_cast< CDC_Player* >( GetOwner() );
		if ( !pPlayer )
			return;
