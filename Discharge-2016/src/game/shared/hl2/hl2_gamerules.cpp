...
#if defined(DISCHARGE_DLL) || defined(DISCHARGE_CLIENT_DLL)
	#include "discharge/cs_weapon_parse.h"
	#include "discharge/cs_blackmarket.h"
...
#if defined(DISCHARGE_DLL) || defined(DISCHARGE_CLIENT_DLL)
int CHalfLife2::GetBlackMarketPriceForWeapon(int iWeaponID)
{
	if (m_pPrices == NULL)
	{
...
int CHalfLife2::GetBlackMarketPreviousPriceForWeapon(int iWeaponID)
{
	if (m_pPrices == NULL)
	{
		GetBlackMarketPriceList();
...
const weeklyprice_t *CHalfLife2::GetBlackMarketPriceList(void)
{
...
void CHalfLife2::SetBlackMarketPrices(bool bSetDefaults)
{
...
//		def.AddAmmoType("Extinguisher",		DMG_BURN,					TRACER_NONE,			0,	0, 100, 0, 0 );
		...
		def.AddAmmoType(AMMO_TYPE_HEGRENADE, DMG_BLAST, TRACER_LINE, 0, 0, 1/*max carry*/, 1, 0);
		def.AddAmmoType(AMMO_TYPE_FLASHBANG, 0, TRACER_LINE, 0, 0, 2/*max carry*/, 1, 0);
		def.AddAmmoType(AMMO_TYPE_SMOKEGRENADE, 0, TRACER_LINE, 0, 0, 1/*max carry*/, 1, 0);
#endif
