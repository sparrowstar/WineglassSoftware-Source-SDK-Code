...
#if defined(DISCHARGE_DLL) || (DISCHARGE_CLIENT_DLL)
float	g_HLlateralBob;
float	g_HLverticalBob;
#else
float	g_lateralBob;
float	g_verticalBob;
#endif
...
#if defined(DISCHARGE_DLL) || (DISCHARGE_CLIENT_DLL)
	g_HLlateralBob = speed*0.005f;
	g_HLlateralBob = g_HLlateralBob*0.3 + g_HLlateralBob*0.7*sin(cycle);
	g_HLlateralBob = clamp(g_HLlateralBob, -7.0f, 4.0f);
#else
...
#if defined(DISCHARGE_DLL) || (DISCHARGE_CLIENT_DLL)
	VectorMA(origin, g_HLverticalBob * 0.1f, forward, origin);

	// Z bob a bit more
	origin[2] += g_HLverticalBob * 0.1f;

	// bob the angles
	angles[ROLL] += g_HLverticalBob * 0.5f;
	angles[PITCH] -= g_HLverticalBob * 0.4f;

	angles[YAW] -= g_HLlateralBob  * 0.3f;

	VectorMA(origin, g_HLlateralBob * 0.8f, right, origin);
#else
